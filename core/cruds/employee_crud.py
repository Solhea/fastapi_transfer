...
def create_employee(db: Session, employee: EmployeeCreate):
    db_employee = EmployeeTable(
        first_name=employee.firstName, last_name=employee.lastName, email=employee.email, phone=employee.phone, address=employee.address,  department=employee.department, lat=employee.lat, lng=employee.lng, is_picked=employee.is_picked, operation_id=employee.operation_id)
    db.add(db_employee)
    try:
        db.commit()
